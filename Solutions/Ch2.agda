...
isOne zero = false
isOne (suc zero) = true
isOne (suc (suc n)) = false

isTwo : ℕ → Bool
isTwo zero = false
isTwo (suc zero) = false
isTwo (suc (suc zero)) = true
isTwo (suc (suc (suc n))) = false

isThreeOrFour : ℕ → Bool
isThreeOrFour zero = false
isThreeOrFour (suc zero) = false
isThreeOrFour (suc (suc zero)) = false
isThreeOrFour (suc (suc (suc zero))) = true
isThreeOrFour (suc (suc (suc (suc zero)))) = true
isThreeOrFour (suc (suc (suc (suc (suc n))))) = false
...
isZeroOrNine : ℕ → Bool
isZeroOrNine zero = true
isZeroOrNine (suc (suc (suc (suc (suc (suc (suc (suc (suc zero))))))))) = true
isZeroOrNine _ = false

isBetweenFiveAndEight : ℕ → Bool
isBetweenFiveAndEight (suc (suc (suc (suc (suc zero))))) = true
isBetweenFiveAndEight (suc (suc (suc (suc (suc (suc zero)))))) = true
isBetweenFiveAndEight (suc (suc (suc (suc (suc (suc (suc zero))))))) = true
isBetweenFiveAndEight (suc (suc (suc (suc (suc (suc (suc (suc zero)))))))) = true
isBetweenFiveAndEight _ = false
...
_+'_ : ℕ → ℕ → ℕ
n +' m = applyNTimes n suc m

_∸'_ : ℕ → ℕ → ℕ
n ∸' m = applyNTimes n pred m

_*'_ : ℕ → ℕ → ℕ
n *' m = applyNTimes n (λ r → m + r) zero
...
NaryFunction zero A R = R
NaryFunction (suc n) A R = A → NaryFunction n A R

-- a synonym for _≡ᵇ_
binary-equals : NaryFunction 2 ℕ Bool
binary-equals = _≡ᵇ_

-- a synonym for suc
unary-suc : NaryFunction 1 ℕ ℕ
unary-suc = suc

-- a synonym for 12
twelve : {A : Set} → NaryFunction 0 A ℕ -- We can actually use whatever argument type we want here
twelve = 12
...
sum-of f until zero = 0
sum-of f until suc n = f n + sum-of f until n


-- Define the following functions using applyNTimes instead of explicit recursion:

isEven' : ℕ → Bool
isEven' n = applyNTimes n not true

isZero' : ℕ → Bool
isZero' n = applyNTimes n (λ _ → true) false

_<'_ : ℕ → ℕ → Bool
n <' m = isPositive (applyNTimes n pred m)

_≡ᵇ'_ : ℕ → ℕ → Bool
n ≡ᵇ' m = isZero (applyNTimes n pred m) ∧ isZero (applyNTimes m pred n)
...
scan op x stream zero = op x (stream zero)
scan op x stream (suc i) = op (scan op x stream i) (stream (suc i))
...
infinite-sequences-are-unenumerable enumeration = λ i → not (enumeration i i)

cantor-unpairing-helper : ℕ → ℕ → ℕ → Bool → ℕ
cantor-unpairing-helper zero counter cap = λ x-or-y → if x-or-y then counter else cap ∸ counter
cantor-unpairing-helper (suc n) zero cap = cantor-unpairing-helper n (suc cap) (suc cap)
cantor-unpairing-helper (suc n) (suc counter) cap = cantor-unpairing-helper n counter cap

unpair-x unpair-y : ℕ → ℕ
unpair-x n = cantor-unpairing-helper n 0 0 true
unpair-y n = cantor-unpairing-helper n 0 0 false
...
flatten-streams stream-stream = λ i → stream-stream (unpair-x i) (unpair-y i)
...
church-halve n {A} = λ f x → n {(A → A → A) → A} (λ p c → c (p λ a b → b) ((p λ a b → f a))) (λ c → x) λ a b → a
