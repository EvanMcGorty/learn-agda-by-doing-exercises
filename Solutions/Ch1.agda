...
module _ {True : Set} {False : Set}
  ...
  peirces-law-P=True f = proof-of-True

  peirces-law-P=False : {Q : Set} → ((False → Q) → False) → False
  peirces-law-P=False f = f disproof-of-False
...
peirces-law-P=Bool f = false -- true is also a valid solution, but notably it is impossible to use 'f' here.
...
incrementLift : {A : Set} → (A → Bool) → (A → A) → (Bool → A) → (Bool → A)
incrementLift isZero' increment' x false = increment' (x false)
incrementLift isZero' increment' x true = if isZero' (increment' (x false)) then increment' (x true) else x true

increment1 = not
increment2 = incrementLift isZero1 increment1
increment4 = incrementLift isZero2 increment2
increment8 = incrementLift isZero4 increment4
increment16 = incrementLift isZero8 increment8

increment : Uint32 → Uint32
increment = incrementLift isZero16 increment16
...
module ClassicalLogic
  ...
  sym-Or {A} {B} [a-to-b]-to-b b-to-a =
    peirces-law {A} {B} (λ a-to-b → b-to-a ([a-to-b]-to-b a-to-b))

  Not : Set → Set
  Not A = A → False

  lem : {A : Set} → (Not A) Or A
  lem {A} [a-to-false]-to-a =
    peirces-law {A} {False} [a-to-false]-to-a

  dne : {A : Set} → Not (Not A) → A
  dne {A} [a-to-false]-to-false =
    peirces-law {A} {False} (λ a-to-false → disproof-of-False ([a-to-false]-to-false a-to-false))
  ...
  left {A} {B} [a-to-[b-to-false]]-to-false =
    dne {A} λ a-to-false → [a-to-[b-to-false]]-to-false (λ a b → a-to-false a)

  right : {A B : Set} → A And B → B
  right {A} {B} [a-to-[b-to-false]]-to-false =
    dne {B} λ a-to-false → [a-to-[b-to-false]]-to-false (λ a b → a-to-false b)

  demorgan-helper : {A B : Set} → A → B → A And B
  demorgan-helper a b a-to-[b-to-false] = a-to-[b-to-false] a b

  demorgan : {A B : Set} → Not (Not A And Not B) → A Or B
  demorgan {A} {B} not-[not-a-and-not-b] a-to-b =
    dne {B} (λ not-b → not-[not-a-and-not-b] (demorgan-helper {Not A} {Not B} (λ a → not-b (a-to-b a)) not-b))